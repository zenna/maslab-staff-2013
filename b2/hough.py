...
if __name__ == "__main__":
    ...
    try:
        while True:
            orig = billy.get_frame()

            # Convert from BGR to HSV
            processed = cv.CreateImage(cv.GetSize(orig), 8, 1)
            cv.CvtColor(orig, processed, cv.CV_BGR2GRAY)
            ...
            # a = numpy.asarray(storage)
            # import ipdb
            # ipdbself.set_trace()        
            #     print "found", circles.shape()
            #     # print 
            #     import ipb
            #     ipb.set_trace()
            #     # n = np.shape(circles)
            ...
            #     # print circles
            #     # for circle in circles:
            #     #     cv2.circle(img,(circle[0],circle[1]),circle[2],(0,0,255))
            # except:
            #     pass
            # circles  = cv2.HoughCircles(grey, cv.CV_HOUGH_GRADIENT, 3, 100, circles, 200, 100, 5, 16)


            # cv2.HoughCircles(grey, grey, 7,7)
